...
export async function verify(
  ...
  switch (tier) {
    ...
    case '1b': {
      if (!format) {
        return err(
          new VerificationError('Tier 1b verification requires format parameter', { tier }),
        );
      }
      try {
        ...
        if (!parserResult.ok) {
          return err(
            new VerificationError(`Tier 1b: unsupported format "${format}"`, { tier, format }),
          );
        }
        ...
        return ok({
          ...
          method: 'semantic',
          tier,
          detail: passed ? 'Parsed content is semantically equal' : 'Parsed content differs',
        });
      } catch (e) {
        return err(
    ...
    case '2': {
      if (!format) {
        return err(
          new VerificationError('Tier 2 verification requires format parameter', { tier }),
        );
      }
      try {
        const { getParser } = await import('../document/parser.js');
        ...
        if (!parserResult.ok) {
          return err(
            new VerificationError(`Tier 2: unsupported format "${format}"`, { tier, format }),
          );
        }
        const parser = parserResult.value;
        const parsedOriginal = await parser.parse(original);
        ...
        const textNodesOriginal = parsedOriginal.segments
          .filter(s => !s.skippable)
          .map(s => s.text);
        const textNodesRestored = parsedRestored.segments
          .filter(s => !s.skippable)
          .map(s => s.text);
        ...
        return ok({
          ...
          method: 'structural',
          tier,
          detail: passed ? 'Text node arrays are equal' : 'Text node arrays differ',
        });
      } catch (e) {
        return err(
          new VerificationError(
    ...
    case '3': {
      if (!format) {
        return err(
          new VerificationError('Tier 3 verification requires format parameter', { tier }),
        );
      }
      try {
        const { getParser } = await import('../document/parser.js');
        ...
        if (!parserResult.ok) {
          return err(
            new VerificationError(`Tier 3: unsupported format "${format}"`, { tier, format }),
          );
        }
        const parser = parserResult.value;
        const parsedOriginal = await parser.parse(original);
        ...
        const textOriginal = parsedOriginal.segments
          .filter(s => !s.skippable)
          .map(s => s.text)
          .join('');
        const textRestored = parsedRestored.segments
          .filter(s => !s.skippable)
          .map(s => s.text)
          .join('');
