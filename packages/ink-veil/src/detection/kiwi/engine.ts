...
interface KiwiBuilderStatic {
  create: (
    wasmPath: string,
  ) => Promise<{ build: (args: unknown) => Promise<KiwiInstance> }>;
}
...
export class KiwiEngine {
  ...
  async init(modelDir: string): Promise<void> {
    ...
    await Promise.all(
      map(MODEL_FILE_NAMES, async (name) => {
        const buf = await readFile(join(modelDir, name));
        modelFiles[name] = new Uint8Array(
          buf.buffer,
          buf.byteOffset,
          buf.byteLength,
        );
