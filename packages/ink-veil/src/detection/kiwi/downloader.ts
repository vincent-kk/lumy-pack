...
function validateModelName(model: string): void {
  if (!/^[a-zA-Z0-9_-]+$/.test(model)) {
    throw new Error(
      `Invalid model name '${model}'. Only alphanumeric, hyphen, and underscore allowed.`,
    );
  }
}
...
async function downloadTo(
  ...
  if (!entry) {
    process.stderr.write(
      `ink-veil: Unknown model '${model}'. Available: ${Object.keys(MODEL_REGISTRY).join(", ")}\n`,
    );
    return null;
  }
  ...
  if (!entry.url.startsWith("https://")) {
    process.stderr.write(
      `ink-veil: Refusing non-HTTPS model URL: ${entry.url}\n`,
    );
    return null;
  }
  ...
  try {
    ...
    process.stderr.write(
      `ink-veil: Downloading Kiwi model (${entry.sizeLabel})...\n`,
    );
    ...
    process.stderr.write(
      `ink-veil: Kiwi model installed at ${targetDir} (SHA256 verified).\n`,
    );
    return targetDir;
  } catch (err) {
 ...
 */
export async function ensureModelAt(
  model: string,
  targetPath: string,
): Promise<string | null> {
  ...
  process.stderr.write(
    `ink-veil: Model not found at ${targetPath}, downloading...\n`,
  );
