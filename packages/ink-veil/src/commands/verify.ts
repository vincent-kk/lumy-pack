...
export function buildVerifyCommand(): Command {
  const cmd = new Command("verify").description(
    "Verify round-trip fidelity between original and restored files",
  );
      ...
      async (
        ...
        if (!result.ok) {
          process.stderr.write(
            `ink-veil: Verification error: ${result.error.message}\n`,
          );
        ...
        if (!jsonMode) {
          process.stdout.write(
            `Verification ${vr.passed ? "PASSED" : "FAILED"} (tier=${tier}, method=${vr.method})\n`,
          );
