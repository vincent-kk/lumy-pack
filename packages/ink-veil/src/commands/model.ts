...
import {
  ...
  isModelInstalled,
  ensureModel,
} from "../detection/kiwi/downloader.js";
...
export function buildModelCommand(): Command {
    ...
    .action(async () => {
      if (isModelInstalled("kiwi-base")) {
        process.stdout.write(
          `Model already installed: ${resolveModelDir("kiwi-base")}\n`,
        );
  ...
  model
    ...
    .action(async () => {
      if (isModelInstalled("kiwi-base")) {
        process.stdout.write(
          `kiwi-base: installed (${resolveModelDir("kiwi-base")})\n`,
        );
      } else {
        process.stdout.write(
          "kiwi-base: not installed. Run: ink-veil model download\n",
        );
      }
      process.exit(ErrorCode.SUCCESS);
    });
  ...
  model
    ...
    .action(async () => {
      if (isModelInstalled("kiwi-base")) {
        ...
        process.stdout.write(
          "No models installed. Run: ink-veil model download\n",
        );
      }
      process.exit(ErrorCode.SUCCESS);
    });
