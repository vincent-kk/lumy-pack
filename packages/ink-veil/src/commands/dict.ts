...
export function buildDictCommand(): Command {
  ...
  cmd
    ...
    .action(async (dictPath: string, opts: Record<string, unknown>) => {
      ...
      const entry = dict.addEntity(original, category, 'MANUAL', 1.0, 'manual-add');

      await saveDictionary(dict, abs);

      if (jsonMode) {
        process.stdout.write(JSON.stringify({ success: true, entry }, null, 2) + '\n');
      } else {
        process.stdout.write(`Added: ${entry.id} "${original}" [${category}]\n`);
      }
    ...
    .action(async (dictPath: string, opts: Record<string, unknown>) => {
      ...
      if (jsonMode) {
        ...
        for (const e of entries) {
          process.stdout.write(`${e.id}\t${e.category}\t"${e.original}"\t${e.token}\n`);
        }
      }
  ...
  cmd
    ...
    .option('-o, --output <path>', 'Output path for merged dictionary', './merged.json')
    .option('--strategy <strategy>', 'Merge strategy: keep-mine, keep-theirs, rename', 'keep-mine')
    .option('--json', 'Output structured JSON')
    .action(async (dictA: string, dictB: string, opts: Record<string, unknown>) => {
      const jsonMode = Boolean(opts['json']);
      const strategy = String(opts['strategy'] ?? 'keep-mine') as 'keep-mine' | 'keep-theirs' | 'rename';
      ...
      if (jsonMode) {
        ...
        process.stdout.write(`Merged → ${outputPath} (added=${result.added}, skipped=${result.skipped}, renamed=${result.renamed}, conflicts=${result.conflicts.length})\n`);
      }
