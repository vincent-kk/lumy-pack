...
function extractYamlSegments(obj: unknown, path: string, segments: TextSegment[]): void {
  if (typeof obj === 'string') {
    ...
    obj.forEach((item, i) => extractYamlSegments(item, `${path}[${i}]`, segments));
...
function applyYamlSegments(
  ...
  if (typeof obj === 'string') {
    ...
    return obj.map((item, i) => applyYamlSegments(item, `${path}[${i}]`, segmentMap));
  } else if (obj !== null && typeof obj === 'object') {
    const result: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
      result[key] = applyYamlSegments(value, path ? `${path}.${key}` : key, segmentMap);
    }
    return result;
  }
  return obj;
