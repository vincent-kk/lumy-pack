...
function extractTomlSegments(
  ...
  if (isString(obj)) {
    ...
    obj.forEach((item, i) =>
      extractTomlSegments(item, `${path}[${i}]`, segments),
    );
...
function applyTomlSegments(
  ...
  if (isString(obj)) {
    ...
    return obj.map((item, i) =>
      applyTomlSegments(item, `${path}[${i}]`, segmentMap),
    );
  } else if (obj !== null && typeof obj === "object") {
    const result: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
      result[key] = applyTomlSegments(
        value,
        path ? `${path}.${key}` : key,
        segmentMap,
      );
...
export class TomlParser implements FormatParser {
  ...
  async reconstruct(parsedDoc: ParsedDocument): Promise<Buffer> {
    ...
    const text = TOML.stringify(
      updated as Parameters<typeof TOML.stringify>[0],
      { newline: "\n" },
    );
