...
export class TextParser implements FormatParser {
  ...
  async parse(buffer: Buffer, encoding?: string): Promise<ParsedDocument> {
    ...
    const detectedEncoding = encoding ?? (bom ? bom : (chardet.detect(buffer) ?? 'utf-8'));
    const normalizedEncoding = detectedEncoding.toLowerCase().replace('utf-8-bom', 'utf-8');
    ...
    if (this.format === 'md') {
      ...
      while ((match = codeBlockRegex.exec(text)) !== null) {
        ...
        segments.push({
          text: match[0],
          position: { type: 'offset', start: match.index, end: match.index + match[0].length },
          skippable: true,
        });
  ...
  async reconstruct(parsed: ParsedDocument): Promise<Buffer> {
    ...
    const normalizedEncoding = parsed.encoding.toLowerCase().replace('utf-8-bom', 'utf-8');
