...
describe('verify — Tier 1b (semantic)', () => {
  ...
  it('identical JSON content with different formatting → passed: true', async () => {
    const original = Buffer.from(JSON.stringify({ name: '홍길동', phone: '010-1234-5678' }, null, 2), 'utf-8');
    const restored = Buffer.from(JSON.stringify({ name: '홍길동', phone: '010-1234-5678' }), 'utf-8');
...
describe('verify — Tier 2 (structural)', () => {
  it('identical HTML → passed: true', async () => {
    const html = Buffer.from('<html><body><p>홍길동</p></body></html>', 'utf-8');
  ...
  it('different HTML text content → passed: false', async () => {
    const original = Buffer.from('<html><body><p>홍길동</p></body></html>', 'utf-8');
    const restored = Buffer.from('<html><body><p>PER_001</p></body></html>', 'utf-8');
  ...
  it('without format → Result.err', async () => {
    const result = await verify(Buffer.from('<p>a</p>'), Buffer.from('<p>a</p>'), '2');
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error.message).toMatch(/format/i);
  });
