...
describe('ManualEngine', () => {
  ...
  describe('string pattern rules', () => {
    it('detects a single string occurrence at correct offset', () => {
      ...
      const spans = engine.detect(text, [{ pattern: 'Project-Alpha', category: 'PROJECT' }]);
    ...
    it('returns empty array when pattern not found', () => {
      const spans = engine.detect('Hello world', [{ pattern: 'Project-Alpha', category: 'PROJECT' }]);
      expect(spans).toHaveLength(0);
    });

    it('ignores empty string patterns', () => {
      const spans = engine.detect('Hello world', [{ pattern: '', category: 'PROJECT' }]);
      expect(spans).toHaveLength(0);
    });
  });
  ...
  describe('regex pattern rules', () => {
    it('matches invoice number pattern', () => {
      ...
      const spans = engine.detect(text, [{ pattern: /INV-\d{8}/g, category: 'INVOICE' }]);
    ...
    it('works with non-global regex (auto-adds global flag)', () => {
      ...
      const spans = engine.detect(text, [{ pattern: /특수코드-A\d/, category: 'SERIAL' }]);

      expect(spans).toHaveLength(2);
    });

    it('returns empty array when regex has no match', () => {
      const spans = engine.detect('no match here', [{ pattern: /INV-\d{8}/g, category: 'INVOICE' }]);
      expect(spans).toHaveLength(0);
    });
  });
