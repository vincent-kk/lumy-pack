...
function calculateF1(
  expected: DetectionSpan[],
  actual: DetectionSpan[],
): AccuracyResult {
  const expectedSet = new Set(
    expected.map((s) => `${s.start}:${s.end}:${s.text}`),
  );
  ...
  const recall =
    expectedSet.size === 0 ? 1.0 : truePositives / expectedSet.size;
  const f1 =
    precision + recall === 0
      ? 0
...
const koreanPiiSamples = [
  ...
  {
    name: "주민등록번호 (하이픈 없음)",
    text: "주민번호 9001011234567로 확인했습니다.",
  },
  ...
  {
    name: "이메일 (복잡)",
    text: "이메일: user.name+tag@company.co.kr로 보내주세요.",
  },
  ...
  {
    name: "복합 (이름+전화)",
    text: "김영희의 번호는 010-9876-5432이며, 이메일은 kim@test.com입니다.",
  },
  {
    name: "복합 (주소+주민)",
    text: "서울시 마포구 합정동에 거주하는 박지수(890203-2345678)씨.",
  },
  {
    name: "복합 (다수 이메일)",
    text: "admin@foo.com과 user@bar.org에 동시 발송합니다.",
  },
  {
    name: "복합 (전화+계좌)",
    text: "010-1111-2222로 연락 후 우리은행 1002-123-456789로 송금.",
  },
  {
    name: "긴 문장",
    text: "2024년 1월 15일, 서울특별시 서초구에 거주하는 이민호(950315-1234567)씨는 010-5555-6666으로 연락 가능하며, minholee@gmail.com으로도 문의할 수 있습니다.",
  },
  { name: "짧은 텍스트", text: "010-1234-5678" },
  { name: "PII 없음", text: "오늘 날씨가 참 좋습니다." },
  {
    name: "영문 혼합",
    text: "John의 번호는 010-3333-4444이고, john@example.com입니다.",
  },
  {
    name: "다수 엔티티",
    text: "홍길동(010-1234-5678, hong@test.com), 김철수(010-8765-4321, kim@test.com) 참석",
  },
];
...
describe("chunking accuracy regression", () => {
  ...
  it("detectChunked produces identical results to detect for small texts", async () => {
    for (const sample of koreanPiiSamples) {
      ...
      const chunked = await pipeline.detectChunked(sample.text, {
        chunkSize: 65536,
      });
  ...
  for (const sample of koreanPiiSamples) {
    it(`maintains F1 >= 0.95 with small chunks for: ${sample.name}`, async () => {
      ...
      if (baseline.length > 0) {
        expect(
          result.f1,
          `F1 score ${result.f1.toFixed(3)} < 0.95 for: ${sample.name}`,
  ...
  it("handles large text with repeated PII patterns", async () => {
    const segment =
      "홍길동의 번호는 010-1234-5678이고 이메일은 test@example.com입니다. ";
    const largeText = segment.repeat(100);

    const baseline = await pipeline.detect(largeText);
    const chunked = await pipeline.detectChunked(largeText, {
      chunkSize: 512,
      overlap: 64,
    });
    const result = calculateF1(baseline, chunked);
    ...
    expect(
      result.f1,
      `Large text F1: ${result.f1.toFixed(3)}`,
    ).toBeGreaterThanOrEqual(0.95);
  });
});
