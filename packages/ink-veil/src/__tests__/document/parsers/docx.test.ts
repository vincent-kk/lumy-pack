...
describe('DocxParser — Tier 2', () => {
  ...
  it('extracts text from w:t nodes', async () => {
    const buf = await makeDocx('<w:p><w:r><w:t>홍길동</w:t></w:r><w:r><w:t>삼성전자</w:t></w:r></w:p>');
    const parsed = await parser.parse(buf);
  ...
  it('non-text XML structure is not corrupted', async () => {
    const buf = await makeDocx('<w:p><w:pPr><w:jc w:val="center"/></w:pPr><w:r><w:t>내용</w:t></w:r></w:p>');
    const parsed = await parser.parse(buf);
