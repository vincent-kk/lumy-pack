...
describe('veilTextFromSpans', () => {
  it('replaces a single span with its token', () => {
    ...
    const result = veilTextFromSpans('홍길동이 왔다', [
      { start: 0, end: 3, text: '홍길동', category: 'PER', method: 'NER', confidence: 0.95 },
    ], dict);
  ...
  it('replaces multiple non-overlapping spans', () => {
    ...
    const result = veilTextFromSpans(text, [
      { start: 0, end: 3, text: '홍길동', category: 'PER', method: 'NER', confidence: 0.95 },
      { start: 5, end: 9, text: '삼성전자', category: 'ORG', method: 'REGEX', confidence: 1.0 },
    ], dict);
  ...
  it('correct offset substitution for multi-byte Korean characters', () => {
    ...
    const text = 'A 홍길동 B';
    const result = veilTextFromSpans(text, [
      { start: 2, end: 5, text: '홍길동', category: 'PER', method: 'MANUAL', confidence: 1.0 },
    ], dict);
  ...
  it('adds entity to dictionary', () => {
    const dict = Dictionary.create('plain');
    veilTextFromSpans('홍길동', [
      { start: 0, end: 3, text: '홍길동', category: 'PER', method: 'NER', confidence: 0.9 },
    ], dict);
