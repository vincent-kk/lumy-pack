...
describe.skipIf(!hasNerModel)("NER pipeline E2E", () => {
  ...
  it(
    "A: detect — NER 파이프라인 정상 실행 및 엔티티 감지",
    { timeout: 30_000 },
    () => {
  ...
  it(
    "B: veil — NER 포함 veil 결과에 iv- 태그 존재",
    { timeout: 30_000 },
    () => {
      const r = runCli([
        "veil",
  ...
  it(
    "C: unveil — round-trip tokenIntegrity === 1.0",
    { timeout: 30_000 },
    () => {
      const veiledPath = join(VEILED_DIR, "korean-pii.txt");
      expect(existsSync(veiledPath), "B 테스트의 veiled 파일이 필요").toBe(
        true,
      );
  ...
  it("D: NER 포함 시 substitutions >= regex-only", { timeout: 30_000 }, () => {
    ...
    expect(
      regexResult.exitCode,
      `regex-only veil failed: ${regexResult.stderr}`,
    ).toBe(0);
    ...
    if (existsSync(nerResultPath)) {
      ...
      expect(nerResult.exitCode, `NER veil failed: ${nerResult.stderr}`).toBe(
        0,
      );
