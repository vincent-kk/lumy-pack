...
describe("Round-trip by format", () => {
  ...
  for (const { file, format } of formats) {
    it(`${format}: veil -> unveil round-trip`, () => {
      ...
      expect(
        veilResult.exitCode,
        `veil failed for ${format}: ${veilResult.stderr}`,
      ).toBe(0);
      ...
      if (!existsSync(veiledPath)) {
        console.warn(
          `veiled file not written for ${format}, skipping unveil check`,
        );
        return;
      }
      ...
      expect(
        unveilResult.exitCode,
        `unveil failed for ${format}: ${unveilResult.stderr}`,
      ).toBe(0);
      ...
      expect(
        result.tokenIntegrity,
        `${format} tokenIntegrity should be 1.0`,
      ).toBe(1.0);
      ...
      if (existsSync(restoredPath)) {
        ...
        expect(
          restoredText,
          `${format} should not contain iv-tag tokens after unveil`,
        ).not.toMatch(/<iv-\w+ id=["']\d+["']>/);
        expect(
          restoredText,
          `${format} should not contain bare token patterns after unveil`,
        ).not.toMatch(
          /\b(?:PER|ORG|LOC|PHONE|EMAIL|RRN|CARD|ACCOUNT|IP|BRN)_\d{3,}\b/,
        );
      }
    });
  }
...
describe("Tier 4 parser availability", () => {
  ...
  it("LaTeX veil — PII 감지 및 정상 처리", () => {
    ...
    writeFileSync(
      tmpFile,
      "\\section{고객정보}\n\\textbf{홍길동}의 전화번호는 010-1234-5678이다.",
      "utf-8",
    );
