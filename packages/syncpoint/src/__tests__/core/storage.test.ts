  ...
  describe("createArchive", () => {
    ...
    it("creates archive with sourcePath files", async () => {
      ...
      try {
        ...
        const outputPath = join(sandbox.root, "test.tar.gz");
        const files = [
          { name: "archived.txt", sourcePath: sourceFile },
        ];
    ...
    it("handles nested directory paths", async () => {
      ...
      try {
        const outputPath = join(sandbox.root, "test.tar.gz");
        const files = [
          { name: "dir1/dir2/file.txt", content: "nested" },
        ];
        ...
        const content = await readFileFromArchive(outputPath, "dir1/dir2/file.txt");
    ...
    it("creates archive with single file", async () => {
      ...
      try {
        ...
        const files = [
          { name: "only.txt", content: "single file" },
        ];
  ...
  describe("extractArchive", () => {
    it("extracts archive files to destination directory", async () => {
      ...
      try {
        ...
        const file1Content = await readFile(join(destDir, "file1.txt"), "utf-8");
        const file2Content = await readFile(join(destDir, "file2.txt"), "utf-8");
    ...
    it("preserves nested directory structure", async () => {
      ...
      try {
        ...
        const files = [
          { name: "dir1/file.txt", content: "nested content" },
        ];
        await createArchive(files, archivePath);
        ...
        const content = await readFile(join(destDir, "dir1", "file.txt"), "utf-8");
    ...
    it("extracts files with correct content", async () => {
      ...
      try {
        ...
        const extractedContent = await readFile(join(destDir, "test.txt"), "utf-8");
  ...
  describe("readFileFromArchive", () => {
    ...
    it("returns null for non-existent file", async () => {
      ...
      try {
        ...
        const content = await readFileFromArchive(archivePath, "nonexistent.txt");
    ...
    it("reads nested file from archive", async () => {
      ...
      try {
        ...
        const content = await readFileFromArchive(archivePath, "dir1/dir2/nested.txt");
  ...
  describe("listArchiveEntries", () => {
    ...
    it("returns correct paths without ./ prefix normalization", async () => {
      ...
      try {
        ...
        expect(entries.some(e => e === "file.txt" || e === "./file.txt")).toBe(true);
      } finally {
        await sandbox.cleanup();
      }
    });
