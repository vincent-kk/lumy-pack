...
import { loadTemplate, listTemplates, executeStep, runProvision } from "../../core/provision.js";
...
vi.mock("node:child_process", () => ({
  exec: vi.fn((cmd: string, opts: unknown, cb: (err: Error | null, stdout: string, stderr: string) => void) => {
...
`;
      ...
      await expect(loadTemplate(templatePath)).rejects.toThrow("Invalid template");
    });
  });
...
steps:
  ...
  - name: should-not-run
    command: echo hello`,
      ...
      expect(results.some((r) => r.name === "fail-step" && r.status === "failed")).toBe(true);
...
steps:
  ...
  - name: should-run
    command: echo hello`,
      ...
      expect(results.some((r) => r.name === "fail-step" && r.status === "failed")).toBe(true);
      expect(results.some((r) => r.name === "should-run" && r.status === "success")).toBe(true);
    });
