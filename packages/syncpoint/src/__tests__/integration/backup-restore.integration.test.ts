...
describe('Backup-Restore Integration Tests', () => {
  ...
  it('should create a valid backup archive', async () => {
    ...
    expect(result.metadata.summary.fileCount).toBe(
      result.metadata.files.length,
    );
  });
  ...
  it('should complete full backup+restore roundtrip', async () => {
    ...
    expect(
      backupResult.metadata.files.some((f) => f.path.includes('.zshrc')),
    ).toBe(true);
    ...
    expect(restoreResult.restoredFiles.some((f) => f.includes('.zshrc'))).toBe(
      true,
    );
  ...
  it('should detect modified files during restore', async () => {
    ...
    const overwriteActions = plan.actions.filter(
      (a) => a.action === 'overwrite',
    );
