...
describe('E7: Cross-file move detection via blame -C -C', { timeout: 30000 }, () => {
  ...
  it('featureFlags.astDiff is false when isAstAvailable is mocked false', async () => {
    repo.commit({ 'src/helpers.ts': HELPERS_ORIGINAL }, 'feat: add date helpers');
    repo.moveFile('src/helpers.ts', 'src/date-utils.ts', 'refactor: move helpers to date-utils');
  ...
  it('trace on multiple lines of moved file all return commit nodes', async () => {
    ...
    expect(result.nodes.length).toBeGreaterThanOrEqual(1);
    expect(result.nodes[0].type).toMatch(/^(original_commit|cosmetic_commit)$/);
  ...
  it('trace works after move with an additional commit on top', async () => {
    ...
    repo.commit({ 'src/other.ts': 'export const version = 1;\n' }, 'chore: add version');
