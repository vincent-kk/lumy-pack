...
describe('E9: Range trace deduplication', { timeout: 30000 }, () => {
  ...
  it('range spanning two commits produces at least two unique commit SHAs', async () => {
    ...
    repo.checkout('main');
    const merge1 = repo.merge(
      'feature/part1',
      'Merge pull request #10 from feature/part1',
    );
    ...
    repo.checkout('main');
    const merge2 = repo.merge(
      'feature/part2',
      'Merge pull request #20 from feature/part2',
    );
  ...
  it('range within a single commit produces only one commit node', async () => {
    const lines = Array.from(
      { length: 10 },
      (_, i) => `export const VAR_${i + 1} = ${i + 1};`,
    );

    repo.commit({ 'src/config.ts': makeConfig(lines) }, 'chore: initial');
    ...
    const mergeCommit = repo.merge(
      'feature/all-vars',
      'Merge pull request #5 from feature/all-vars',
    );
  ...
  it('range trace result contains PR nodes for each unique commit', async () => {
    ...
    const mergeA = repo.merge(
      'feature/a',
      'Merge pull request #31 from feature/a',
    );
    ...
    const mergeB = repo.merge(
      'feature/b',
      'Merge pull request #32 from feature/b',
    );
    ...
    prMap.set(
      ...
      createPRInfo({
        number: 32,
        mergeCommit: mergeB,
        url: 'https://github.com/test/repo/pull/32',
      }),
    );
    const adapter = createMockPlatformAdapter({ prMap });
    mockDetectPlatform.mockResolvedValue({
  ...
  it('single-line trace (no endLine) returns one commit node', async () => {
    ...
    const mergeCommit = repo.merge(
      'feature/host',
      'Merge pull request #1 from feature/host',
    );
