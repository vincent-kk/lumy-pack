...
# drift-sync — Structural Drift Sync

Detect deviations between the current project structure and fractal principles, then
apply targeted corrections. `drift-analyzer` scans and classifies drift items,
...
`drift-detect` identifies all drift items. Items are classified by severity:
...
```
/filid:drift-sync [path] [--severity critical|high|medium|low] [--dry-run] [--auto-approve]
```
...
/filid:drift-sync --severity high

# Inspect drift without making changes
/filid:drift-sync --dry-run

# CI automated correction (critical only)
/filid:drift-sync --severity critical --auto-approve
