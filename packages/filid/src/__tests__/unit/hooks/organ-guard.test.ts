...
describe('organ-guard', () => {
  ...
  it('should block CLAUDE.md in nested organ directories', () => {
    const input: PreToolUseInput = {
      ...baseInput,
      tool_input: { file_path: '/app/src/components/ui/CLAUDE.md', content: '# UI' },
    };
    const result = guardOrganWrite(input);
    expect(result.continue).toBe(false);
  });
  ...
  it('should allow non-CLAUDE.md files in organ directories', () => {
    const input: PreToolUseInput = {
      ...baseInput,
      tool_input: { file_path: '/app/src/utils/helper.ts', content: 'export {}' },
    };
    const result = guardOrganWrite(input);
    expect(result.continue).toBe(true);
  });
  ...
  it('should pass through Edit tool calls', () => {
    const input: PreToolUseInput = {
      ...baseInput,
      tool_name: 'Edit',
      tool_input: { file_path: '/app/src/utils/CLAUDE.md', new_string: 'updated' },
    };
    const result = guardOrganWrite(input);
    expect(result.continue).toBe(true);
  });
  ...
  it('should detect all known organ directory names', () => {
    const organDirs = ['components', 'utils', 'types', 'hooks', 'helpers', 'lib', 'styles', 'assets', 'constants'];
