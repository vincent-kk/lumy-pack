...
describe('context-injector', () => {
  it('should inject FCA-AI context reminder', () => {
    const result = injectContext(baseInput);
    expect(result.continue).toBe(true);
    expect(result.hookSpecificOutput?.additionalContext).toBeDefined();
    expect(result.hookSpecificOutput?.additionalContext).toContain('FCA-AI');
  });

  it('should include organ directory awareness', () => {
    const result = injectContext(baseInput);
    const ctx = result.hookSpecificOutput?.additionalContext ?? '';
    expect(ctx).toContain('Organ');
  });

  it('should include CLAUDE.md 100-line limit reminder', () => {
    const result = injectContext(baseInput);
    const ctx = result.hookSpecificOutput?.additionalContext ?? '';
    expect(ctx).toContain('100');
  });

  it('should include 3+12 rule reminder', () => {
    const result = injectContext(baseInput);
    const ctx = result.hookSpecificOutput?.additionalContext ?? '';
    expect(ctx).toContain('15');
  });

  it('should include current working directory in context', () => {
    const result = injectContext(baseInput);
    const ctx = result.hookSpecificOutput?.additionalContext ?? '';
    expect(ctx).toContain('/workspace/project');
  });

  it('should always continue (never block user prompts)', () => {
    const result = injectContext(baseInput);
    expect(result.continue).toBe(true);
  });
});
