...
import { buildFractalTree, findNode, getDescendants } from '../../core/fractal-tree.js';
import { classifyNode, isOrganDirectory } from '../../core/organ-classifier.js';
import { validateClaudeMd, validateSpecMd } from '../../core/document-validator.js';
...
describe('fractal-init pipeline', () => {
  ...
  const entries = [
    { name: 'root', path: '/project', type: 'directory' as const, hasClaudeMd: true, hasSpecMd: false },
    { name: 'auth', path: '/project/auth', type: 'directory' as const, hasClaudeMd: true, hasSpecMd: true },
    { name: 'components', path: '/project/auth/components', type: 'organ' as const, hasClaudeMd: false, hasSpecMd: false },
    { name: 'utils', path: '/project/auth/utils', type: 'organ' as const, hasClaudeMd: false, hasSpecMd: false },
    { name: 'payment', path: '/project/payment', type: 'directory' as const, hasClaudeMd: true, hasSpecMd: true },
    { name: 'helpers', path: '/project/payment/helpers', type: 'organ' as const, hasClaudeMd: false, hasSpecMd: false },
  ];

  it('should build a fractal tree from directory entries', () => {
    const tree = buildFractalTree(entries, '/project');
  ...
  it('should reject CLAUDE.md exceeding 100 lines', () => {
    const longContent = Array.from({ length: 101 }, (_, i) => `Line ${i + 1}`).join('\n');
    const validation = validateClaudeMd(longContent);

    expect(validation.valid).toBe(false);
    expect(validation.violations.some(v => v.rule === 'line-limit')).toBe(true);
  });
