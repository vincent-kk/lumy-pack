...
Claude Code 프로젝트의 구조와 문서를 자동으로 관리하는 플러그인.

코드베이스가 커지면 AI 에이전트가 맥락을 잃고, 문서는 코드와 어긋나고, 디렉토리 구조는 일관성을 잃는다. filid는 이 문제를 **프랙탈 아키텍처(FCA-AI)** 기반의 자동화된 규칙 시행으로 해결한다.
...
설치 후 별도 설정 없이 모든 컴포넌트(Skills, MCP, Agents, Hooks)가 자동 등록된다.

---

## 사용법

filid 스킬은 **LLM 프롬프트**이지, CLI 명령어가 아니다. Claude Code 안에서 자연어로 대화하듯 호출한다. `--fix` 같은 플래그도 LLM이 해석하는 힌트이므로, 자연어로 써도 동일하게 동작한다.
...
프로젝트 디렉토리를 스캔하여 각 모듈에 `CLAUDE.md` 경계 문서를 생성한다. `components/`, `utils/` 같은 유틸리티 디렉토리(organ)는 자동으로 건너뛴다.
...
CLAUDE.md 100줄 초과, 3-tier 경계 섹션 누락, organ 디렉토리 내 CLAUDE.md 존재 등을 검출한다.
...
코드 변경으로 인한 구조적 drift를 감지하고, 해당 모듈의 CLAUDE.md/SPEC.md를 갱신한다. 내부적으로 `drift-detect` MCP 도구를 사용한다.
...
경계 검사 → 문서 검증 → 의존성 분석 → 테스트 메트릭 → 복잡도 평가 → 최종 판정 순서로 진행된다.

### AI 코드 리뷰

가장 강력한 기능. 다중 페르소나 합의체가 3단계로 코드를 리뷰한다.
...
산출물은 `.filid/review/<branch>/`에, 기술 부채는 `.filid/debt/`에 저장된다.
